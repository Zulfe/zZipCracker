...
public class zipCracker {
	...
	public static void zZipCracker() throws ZipException, IOException{
		...
		generatePasswordArray(passwordArray); //put the lines from the dictionary into the array list
		...
		if(zipper.isEncrypted()){ //should return true for all types of encryption
				while(passwordCounter != passwordArray.size()) { //while elements are still in the array list
					zipper.setPassword((String) passwordArray.get(passwordCounter)); //set the password to element position [passwordCounter]
					...
					try {
						 ...
						 JOptionPane.showMessageDialog(null, "The zip has been cracked. The password is " + passwordArray.get(passwordCounter - 1));
						 break;
	...
	//allows a user to pick a .txt dictionary to be converted into elements of the array list to be processed
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static ArrayList generatePasswordArray(ArrayList passwordArray) throws IOException{
